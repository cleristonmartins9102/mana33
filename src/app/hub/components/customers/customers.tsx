import { Modules } from '@/app/data/models/module-model.'
import Styles from './modules-button.module.scss'

type Props = {
	currentModule: any
	setModule: Function
}

export const Customers = ({ currentModule, setModule }: Props) => {
	const handleModuleChange = (newModule: Modules) => {
		setModule(newModule)
	}
	return (
		<div onClick={() => handleModuleChange(Modules.Customers)} className={[Styles.wrap_home, 'cursor-pointer'].join(' ')}>
			<svg className={`w-10 h-10 ${currentModule === Modules.Customers ? "text-teal-700" : "text-gray-500"}`} viewBox="0 0 16 16" version="1.1" fill="currentColor" transform="matrix(-1, 0, 0, 1, 0, 0)" width="40" height="36">
				<g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
				<g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
				<g id="SVGRepo_iconCarrier">
					<rect width="16" height="16" id="icon-bound" fill="none"></rect>
					<path d="M16,12.5C16,11.837 15.737,11.201 15.268,10.732C14.799,10.263 14.163,10 13.5,10C11.447,10 8.553,10 6.5,10C5.837,10 5.201,10.263 4.732,10.732C4.263,11.201 4,11.837 4,12.5C4,14.147 4,15 4,15L16,15C16,15 16,14.147 16,12.5ZM3,13L0,13C0,13 0,12.147 0,10.5C0,9.837 0.263,9.201 0.732,8.732C1.201,8.263 1.837,8 2.5,8L6.536,8C6.754,8.376 7.031,8.714 7.355,9L6.5,9C4.567,9 3,10.567 3,12.5L3,13ZM10,3C11.656,3 13,4.344 13,6C13,7.656 11.656,9 10,9C8.344,9 7,7.656 7,6C7,4.344 8.344,3 10,3ZM6.126,6.997C6.084,6.999 6.042,7 6,7C4.344,7 3,5.656 3,4C3,2.344 4.344,1 6,1C7.026,1 7.932,1.516 8.473,2.302C7.022,2.903 6,4.333 6,6C6,6.344 6.044,6.679 6.126,6.997Z"></path>
				</g>
			</svg>
			<div className={Styles.wrap_legend}>
				<span className={`text-gray-700 ${currentModule === Modules.Customers ? "font-bold" : null}`}>Clientes</span>
			</div>
		</div>
	)
}